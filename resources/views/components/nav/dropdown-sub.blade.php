...
<div class="relative" x-data="{ open: false }" @mouseover.away="open = false" @close.stop="open = false">
    ...
    <div x-show="open"
            ...
            class="absolute z-50 mt-2 {{ $width }} rounded-md shadow-lg -top-3 left-full {{ $dropdownClasses }}"
            style="display: none;"
            >
        <div @click.away="open = false" class="rounded-md ring-1 ring-black ring-opacity-5 {{ $contentClasses }}">
            {{ $content }}
        </div>
    </div>
</div>
